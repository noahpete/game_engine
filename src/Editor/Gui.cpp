...
void Gui::ShowProperties()
{
    ...
    for (Component* component : sSelectedActor->GetComponents())
    {
        ...
        if (ImGui::TreeNodeEx(component->Type.c_str(), comp_flags))
        {
            if (component->Ref["type"].isString())
                ImGui::Text("Type: %s", component->Ref["type"].cast<std::string>().c_str());

            for (luabridge::Iterator iter(component->Ref); !iter.isNil(); ++iter)
            {
                ...
                if (key.isString())
                {
                    ...
                    else if (value.isString())
                    {
                        ...
                        if (ImGui::InputText(key.cast<std::string>().c_str(), buffer, sizeof(buffer)))
                            component->Ref[key] = std::string(buffer);
                    }
